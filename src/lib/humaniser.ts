export function humanizeNoorainQuestion(text: string, surahName: string) {
  ...
  result = result.replace(/^which of these is true\??/i, "Okay, which one matches what we just read?");
  ...
  if (!/(i was reading|let me quiz you|wait|okay|hmm|with you)/i.test(result)) {
    result = `I was reading ${surahName} with you. ${result.charAt(0).toUpperCase()}${result.slice(1)}`;
  }
...
export function humanizeTafsirNote(text: string): string {
  ...
  if (result && !/(subhanallah|mashallah|you know|ibn kathir|this is actually|actually,|so,)/i.test(result.slice(0, 40))) {
